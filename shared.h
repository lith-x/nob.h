...
// TODO: we should test on C++ compilers too
    ...
    #if defined(_MSC_VER)
        #define nob_cc_flags(cmd) nob_cmd_append(cmd, "/W4", "/nologo", "/D_CRT_SECURE_NO_WARNINGS", "-I.")
    #else
        #define nob_cc(cmd) nob_cmd_append(cmd, "cc", "-x", "c++")
        #define nob_cc_flags(cmd) nob_cmd_append(cmd, "-Wall", "-Wextra", "-Wno-missing-field-initializers", "-Wswitch-enum", "-ggdb", "-I.");
    #endif
#else // __cplusplus
    #if defined(_MSC_VER)
        #define nob_cc_flags(cmd) nob_cmd_append(cmd, "/W4", "/nologo", "/D_CRT_SECURE_NO_WARNINGS", "-I.")
    ...
    // "-D_POSIX_C_SOURCE=200112L" hides required symbols on FreeBSD
        #define nob_cc_flags(cmd) nob_cmd_append(cmd, "-Wall", "-Wextra", "-Wswitch-enum", "-std=c99", "-ggdb", "-I.");
    #else
        #define nob_cc_flags(cmd) nob_cmd_append(cmd, "-Wall", "-Wextra", "-Wswitch-enum", "-std=c99", "-D_POSIX_C_SOURCE=200112L", "-ggdb", "-I.");
    #endif
#endif // __cplusplus
...
NOBDEF bool build_tool(Cmd *cmd, Procs *procs, const char *bin_path, const char *src_path, const char *src)
{
    if (!write_entire_file(src_path, src, strlen(src))) return 1;

    if (!spec(cmd, .output = bin_path, .inputs = strs(src_path), .default_flags = true)) return 1;
